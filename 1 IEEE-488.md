...
The IEEE-488 byte transfer layer is based on the transmission of byte streams from one sender to one or more receivers. Any number of additional bus participants can remain silent.

For each byte stream, there can be a different sender and different receivers. For example, it's possible for A to send 10 bytes to B and C, then B sends 5 bytes to A and C, and finally A sends 20 bytes just to C.

In order to assign these roles to the bus participants, there is one participant that is the designated "controller". The controller can send commands that all other bus participants have to receive and interpret.
