...
When the controller wants to send a command, it pulls the ATN ("Attention") line. As long as ATN = 1, all devices are receivers and participate in the 3-wire-handshake. Then the controller sends the command data like any other transmission, and releases ATN afterwards.
...
On the PET side, the Commodore version of IEEE-488 implements timeouts though. By default, there are two steps that can time out:
...
* Since it is the receiver that starts the transmission of a byte, and the sender that has to react within 64 µs, there might not be enough time for the sender to actually retrieve the next byte e.g. from disk. Therefore, the sender is required to retrieve the byte during the transmission of the previous byte, e.g. while during DAV = 1, where there are no timeouts.
* Also, the timer starts running as soon as the PET releases NRFD. In theory, there could be more receivers next to the PET that have to release NRFD as well before the sender is even allowed to send the data. So this timeout will easily break this case.
	
Because of the latter problem, but mostly also for compatibility with non-Commodore IEEE-488 devices, timeouts can be globally disabled using the KERNAL call "SETTMO" ($FFA2) with A = $80.

In addition to being able to recover from malfunctioning devices, timeouts are also an implicit communication channel: If a sender is asked to make a transmission, but doesn't actually have any data, it will signal this by timing out.

Therefore, turning off the timeout will break "DEVICE NOT PRESENT" and "FILE NOT FOUND" detection for Commodore drives on layer 3 (see the next article in this series).
