...
The basic byte transfer protocol of IEEE-488 is based on transmissions of byte streams from one sender to one or more receivers. Additional bus participants will remain silent. Any bus participant can be a sender or receiver for one transmission, and for the next transmissions, the roles can be completely different.
...
In **data mode**, which we have just seen, for each byte stream, one sender transmits data to any number of receivers. There is also **attention mode**: One particiant can start a transmission to **all** other participants at any time.


The protocol described so far allows sending byte streams between one sender and any number of receivers. For every byte stream, there can be a different sender and different receivers, but how do you designate the sender and receivers?

On a IEEE-488, one bus participant, the computer, is the "controller", and all others are "devices". The controller can take over the bus at any time to send command bytes that all devices will actively receive. These command bytes tell the devices who is currently supposed to send and who is supposed to receive.

When the controller wants to send a command byte, it pulls the ATN ("Attention") line. As long as ATN = 1, all devices are receivers and participate in the 3-wire-handshake. Then the controller sends the command byte and releases ATN.

If the controller wants to send several command bytes at a time, releasing ATN between bytes is optional.
...
The sender timeout is problematic, since the timer starts running as soon as the PET releases NRFD. In theory, there could be more receivers next to the PET that have to release NRFD as well before the sender is even allowed to send the data. So this timeout will easily break this case.
	
Because of this, as well as for compatibility with non-Commodore IEEE-488 devices, timeouts can be globally disabled using the KERNAL call "SETTMO" ($FFA2) with A = $80. On layer 3, this will break "DEVICE NOT PRESENT" and "FILE NOT FOUND" detection for Commodore drives though (see Part 2).
